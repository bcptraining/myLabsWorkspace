...
STORAGE_ALLOWED_LOCATIONS = ('s3://vdw-dev-ingest/loadingdatalabs/',
  ...
  -- 's3://vdw-dev-ingest/external_table_lab/csv/',
  -- 's3://vdw-dev-ingest/external_table_lab/json/',
...
FILE_FORMAT = CSV_ET_LAB_FILEFORMAT; 

LIST @EXTERNALDB.ET.AWS_ET_LAB_CSV_STAGE; 
select * FROM HRMS.HR.LOCATIONS WHERE COUNTRY_ID = 'AU';
use schema hr;
show tables;

COPY INTO @EXTERNALDB.ET.AWS_ET_LAB_CSV_STAGE FROM(SELECT * FROM 
HRMS.HR.LOCATIONS WHERE COUNTRY_ID ='AU') 
HEADER =TRUE 
OVERWRITE=TRUE ;
...
T.$5, 
T.$6 
FROM @EXTERNALDB.ET.AWS_ET_LAB_CSV_STAGE T; --Q5 --Create External table on the Locations data. --Provide optional parameter pattern. --Try to create the table without the file format and check if it succeeds.if it does not then go 
ahead and create the ET with the file format --Describe the External Table and Check what is the first column in the ET --Check what is in the expression for Location_Id. 
CREATE OR REPLACE EXTERNAL TABLE EXT_LOCATIONS 
( 
LOCATION_ID        
VARCHAR   AS (VALUE:c1::VARCHAR), 
STREET_ADDRESS     VARCHAR   AS (VALUE:c2::VARCHAR), 
POSTAL_CODE        
VARCHAR   AS (VALUE:c3::VARCHAR), 
CITY               
VARCHAR   AS (VALUE:c4::VARCHAR), 
STATE_PROVINCE     VARCHAR   AS (VALUE:c5::VARCHAR), 
COUNTRY_ID         
VARCHAR   AS (VALUE:c6::VARCHAR) 
) 
...
DESCRIBE TABLE HRMS.HR.LOCATIONS; --Q6 --Select distinct field name and Max row number from the ET --Locate the MD5 values of all  the file locations_US.csv associated with the ET by selecting 
data from Information Schema view --Which Information Schema view will you  go to see registration history 
...
SELECT * FROM 
TABLE(INFORMATION_SCHEMA.EXTERNAL_TABLE_FILE_REGISTRATION_HISTORY('EXT 
_LOCATIONS')); --Assignment 02 -Enable Auto-Refresh ------------------------------------ --Q1 --Add 1 new file (locations_JP.csv) to the path of the External table, wait 5 mins  and check to 
see if the ET is able to register the file. 
Upload the file locations_JP.csvmanually to the s3 folder,files available in resources section. 
s3://<student_folder_name>/external_table_lab/csv/ --Q2 --Manually run a refresh and check if the ET is able to register the file. What message do you 
see in the status field 
ALTER EXTERNAL TABLE EXT_LOCATIONS REFRESH; 
SHOW EXTERNAL TABLES --Q3 --Enable Event Notification on s3.Add Prefix and suffix --Alter the table to make set AUTO_REFRESH to TRUE --Check Pipe status is running in Snowflake --Select the distinct METADATA$FILENAME from the External Table 
...
SELECT DISTINCT METADATA$FILENAME  FROM EXT_LOCATIONS; --Q4 --Add 2  new files(locations_IN.csv and locations_MX.csv)  to the ET folder path and remove 1 
file(locations_UK.csv) and after 5 mins --Check to see if the files that have been added are registered and files that are removed are 
de-registered. --Which table will show you history of registration and de-registration 
SELECT DISTINCT METADATA$FILENAME  FROM EXT_LOCATIONS 
SELECT * FROM 
TABLE(INFORMATION_SCHEMA.EXTERNAL_TABLE_FILE_REGISTRATION_HISTORY('EXT 
_LOCATIONS')); 
--Assignment 3 -Creating Partitioned Parquet External Tables ---------------------------------------------------- --Q1 --Create partitioned ET with Parquet files --Create a folder in AWS  to hold partition data files. --Create sub partition inside the AWS folder based on Country --Upload files to the folder --Create a file format for the parquet files --Create a stage in Snowflake  which points to the partition folder --List the files from the s3 folder in AWS --Use split_part on the file name and exact the CITY column partition data --Create the ET and when creating it ensure that the CITY column is coming from the External 
table path 
...
EXT_LOCATIONS_PARTITIONED; 
CREATE OR REPLACE EXTERNAL TABLE EXT_LOCATIONS_PARTITIONED 
( 
LOCATION_ID        
VARCHAR   AS ($1:LOCATION_ID::VARCHAR), 
STREET_ADDRESS     VARCHAR   AS ($1:STREET_ADDRESS::VARCHAR), 
POSTAL_CODE        
VARCHAR   AS ($1:POSTAL_CODE::VARCHAR), 
CITY               
VARCHAR   AS ($1:CITY::VARCHAR), 
STATE_PROVINCE     VARCHAR   AS ($1:STATE_PROVINCE::VARCHAR), --  COUNTRY_ID         
VARCHAR   AS ($1:COUNTRY_ID::VARCHAR), 
COUNTRY_ID         
varchar AS  split_part(metadata$filename, '/', 3) 
) 
LOCATION=@AWS_ET_PARTITIONED_LAB_PARQUET_STAGE 
PATTERN='.*locations.*[.]parquet' 
FILE_FORMAT = PARQUET_ET_LAB_FILEFORMAT; 
SELECT * FROM EXT_LOCATIONS_PARTITIONED 
WHERE COUNTRY_ID='US' --Assignment 4 -Assignment 03 - JSON External Table ---------------------------------------------------- –Q1 --Create a folder in AWS  to hold JSON data files. --Upload files to the folder --Create a JSON stage in Snowflake  which points to the partition folder --List the files from the s3 folder in AWS --SELECT data  for all columns and examine the data --Create JSON External Table. --Use Lateral Flatten to break the JSON into multiple rows --Create a view  based on the select with the lateral flatten,ensure that double quotes are 
removed. 
...
STORAGE_INTEGRATION = AWS_S3_INT 
URL = 's3://learn2cloud-snowflake/external_table_lab/JSON/' 
...
AS 
SELECT 
Y.VALUE:LOCATION_ID:: INTEGER       
AS LOCATION_ID, 
Y.VALUE:STREET_ADDRESS :: VARCHAR   AS STREET_ADDRESS , 
Y.VALUE:POSTAL_CODE :: VARCHAR      AS POSTAL_CODE, 
Y.VALUE:CITY:: VARCHAR              
AS EMAIL, 
Y.VALUE:STATE_PROVINCE:: VARCHAR    AS PHONE_NUMBER, 
Y.VALUE:COUNTRY_ID:: VARCHAR        
AS COUNTRY_ID 
FROM EXT_LOC_JSON ELJ, 
lateral flatten( input =>LOC_JSON_DATA) Y; 
SELECT * FROM VIEW_EXT_LOC_JSON; 
--Assignment 05 -Streams on External Table ------------------------------------------ --Create a folder in AWS  to hold parquet Streams files. --Upload files to the folder --Create a CSV stage in Snowflake  which points to the parquet Streams folder --List the files from the s3 folder in AWS --SELECT data  for all columns and examine the data --Create an External Table based on the files added. --SELECT data  from the External Table. --Create folder s3://<student_folder_name>/external_table_lab/et_partition_lab/streams/ 
CREATE OR REPLACE FILE FORMAT EXTERNALDB.ET.PARQUET_ET_LAB_FILEFORMAT 
TYPE = PARQUET 
COMPRESSION=NONE; 
CREATE OR REPLACE STAGE EXTERNALDB.ET.AWS_ET_STREAMS_LAB_STAGE 
STORAGE_INTEGRATION = AWS_S3_INT 
URL = 's3://learn2cloud-snowflake/external_table_lab/streams/' 
FILE_FORMAT = PARQUET_ET_LAB_FILEFORMAT; 
LIST @EXTERNALDB.ET.AWS_ET_STREAMS_LAB_STAGE; 
SELECT * FROM @AWS_ET_STREAMS_LAB_STAGE; 
